...
async def start_main_unified_flow(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message.text == "/start" and current_flow_state in active_states_for_reset:
        ...
        for key in [current_flow_state_key, "user_secure_id_z1_s1_v2", "slot_id_z1_s1_v2", "access_key_z1_s1_v2", "integrity_value_s1_v2", "sync_seed_s1_v2", "node_echo_id_s1_v2", "checksum_val_s1_v2"]: # Added new keys
            context.user_data.pop(key, None)

    logger.info(f"[Unified Z1 Flow S1 V2] User {user_id} (Chat: {chat_id}) starting script with final enhancements.")
    ...
    try:
        ...
        await send_delayed_message(context.bot, chat_id, text_a2, delay_before=3.2 - 1.2, show_typing=True)
        ...
        # MODIFICATION 3: Insert [SYS NODE AI::echo] message
        node_echo_id = format(random.randint(0, SEED_MAX_VAL), '04X') # Reusing SEED_MAX_VAL for 4 hex chars
        context.user_data["node_echo_id_s1_v2"] = node_echo_id
        text_b2_echo = (
            f"<code>[SYS NODE AI::echo]</code> Node stabilization task [#{node_echo_id}] acknowledged."
        )
        # Send this new message with its own typing and delay
        await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING) # Standard typing for this new system message
        ...
        # Adjusted delay for B3 if b2_echo takes 1.0s + its own typing. Original delay for B3 was 4.5s after B2.
        # Let's assume B3 follows b2_echo fairly quickly to maintain pressure.
        await send_delayed_message(context.bot, chat_id, text_b3, delay_before=2.0, show_typing=True) # Shortened delay after echo
        logger.info(f"[Unified Z1 Flow S1 V2] User {user_id}: Step B messages (with AI echo) sent.")
        ...
        checksum_val = format(random.randint(0, SEED_MAX_VAL), '04X') # MODIFICATION 2: Generate checksum
        context.user_data["checksum_val_s1_v2"] = checksum_val

        # MODIFICATION 1 & 2 Applied here:
        text_c1 = (
...
logger.info("handlers.step_1 (unified flow v2 with final enhancements) module loaded.")